...
1. **Build** your own Docker image by running
   ...
   docker build \
     ...
     … \
     --tag plate \
     https://github.com/evolutics/code-cleaner-buffet.git#0.8.0
   ```

   For each chosen code cleaner, pass a `--build-arg` with a name-version pair. The build uses the Dockerfile of the repository at above URL, which refers to release `0.8.0` here.
...
```bash
docker build \
  --build-arg addons_linter=1.19.0 \
  --tag plate \
  https://github.com/evolutics/code-cleaner-buffet.git#0.8.0
...
```bash
docker build \
  --build-arg golangci_lint=1.22.0 \
  --tag plate \
  https://github.com/evolutics/code-cleaner-buffet.git#0.8.0
docker run -it --rm plate
...
```bash
docker build \
  --build-arg prettier_ruby=0.17.0 \
  --tag plate \
  https://github.com/evolutics/code-cleaner-buffet.git#0.8.0
docker run -it --rm plate
prettier --version \
...
```bash
docker build \
  --build-arg wemake_python_styleguide=0.13.3 \
  --tag plate \
  https://github.com/evolutics/code-cleaner-buffet.git#0.8.0
docker run -it --rm plate
flake8 --version \
